...
.. class:: TSL2561(i2c, address=0x39)
    ...
    .. method:: read(autogain=False, raw=False)

        Read a measurement from the sensor.

        If ``raw`` is ``False``, returns a single floating point value denoting
        luminosity in lux. If it's ``True``, returns a pair of integer numbers,
        corresponding to the readings of the broadband and infra-red channels,
        respectively.

        If ``autogain`` is set to ``True``, the sensor may repeat the
        measurement with a different gain setting if the result is out of
        bounds for it.
    ...
    .. method:: interrupt(cycles=None, min_value=None, max_value=None)
